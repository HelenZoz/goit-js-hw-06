...
'use strict';
const galleryImgEl = document.querySelector('ul.gallery');

const makeGallery = images.map(({ url, alt }) =>
  `<li><img src=${url} alt=${alt} width=800px height=500px</li>`)
  .join('');

console.log(makeGallery);

galleryImgEl.style.display = 'flex';
galleryImgEl.style.gap = '20px';
galleryImgEl.style.listStyle = 'none';

galleryImgEl.insertAdjacentHTML('afterbegin', makeGallery);
